...
import com.aslifitness.fitracker.onboarding.CreateProfileFragment
...
class UserProfileFragment: Fragment(), DashboardAdapterCallback, WorkoutHistoryCallback, EditProfileBottomSheetCallback {
    ...
    private fun setupListener() {
        ...
        binding.tvWeight.setOnClickListener { openEditBottomSheet(getString(R.string.enter_your_age), userDto?.weight) }
...
//        }
    ...
    private fun onProfileViewStateChanged(state: UserProfileViewState?) {
        when(state) {
            is UserProfileViewState.ShowUserDetail -> showUserDetail(state.userDto)
    ...
    private fun showUserDetail(userDto: UserDto?) {
        userDto?.let {
            this.userDto = it
            binding.userName.setSubtitle(it.name)
            binding.tvAge.setSubtitle(it.age)
            binding.tvWeight.setSubtitle(it.weight)
        }
    }
