...
import com.aslifitness.fitracker.filedownloader.data.MediaItemTag
...
class DownloadTracker(
    ...
    fun toggleDownloadDialogHelper(context: Context, mediaItem: MediaItem, positiveCallback: (() -> Unit)? = null, dismissCallback: (() -> Unit)? = null) {
    ...
    suspend fun getAllDownloadProgressFlow(): Flow<List<Download>> = callbackFlow {
        while (coroutineContext.isActive) {
            trySend(downloads.values.toList())
            delay(1000)
        }
    }
    ...
    private inner class StartDownloadDialogHelper(
        ...
        override fun onPrepared(helper: DownloadHelper) {
            if(helper.periodCount == 0) {
                ...
                val estimatedContentLength = (DEFAULT_BITRATE * mediaItemTag.duration).div(C.MILLIS_PER_SECOND).div(C.BITS_PER_BYTE)
